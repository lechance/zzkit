...
- **Dark mode injection:** content.js injects `html{filter:invert(1)hue-rotate(180deg)}!important` at `document_start` (avoids a white flash on reload). It re-inverts `img,video,canvas,iframe,picture,embed,object,svg:not([role="img"]),[style*="background-image"],[role="img"]` so media isn't negative and inline SVG **logos** (explicit fills, no `role="img"`) keep their original colors — visible on dark. `svg[role="img"]` icon glyphs stay under the page invert (they use `currentColor`, so they render light like the surrounding text). It then re-inverts the **text descendants** of `[style*="background-image"]`/`[role="img"]` (excluding the media elements) so originally-black text inside those containers stays light. It deliberately leaves form controls under the page invert so input text stays visible. Light mode injects `html{color-scheme:light}`.
